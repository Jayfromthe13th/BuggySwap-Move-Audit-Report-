module buggyswap::market {
    ...
    //@audit Vuln: Visability issues. This is default private and needs to be set to where the user can withdraw liquidity. Needs to be public.
    fun remove_liquidity<CoinType>(
        user: &signer,
        lp_size: u64
    ): (u64) acquires OrderStore, CoinStore {
        assert_input_cointype<CoinType>();
    ...
    public fun limit_swap<BaseCoinType, QuoteCoinType>(
        ...
        let order = Order {
            ...
            user_address: address_of(user), // @audit VULN: does not check if account is registered --Needs check
    ...
    public fun cancel_order<BaseCoinType>(
        ...
        // @audit VULN: does not check if order.base_type == type_info::type_of(BaseCoinType) --No Generics type checking
        // @audit VULN: does not check if order.user_address == address_of(user) --Access control 
        deposit_funds<BaseCoinType>(order_store, address_of(user), order.base);

        drop_order(order_store, &order);
    }

        //@audit Vuln: Unbounded Execution in fulfill_orders: - called by add_liquidity 
    ...
    public fun fulfill_orders<CoinType>(): vector<u64> acquires OrderStore, CoinStore {
        ...
        while (true) {
            ...
            //@audit Vuln: fullfill_orders Borrows after execution
    ...
    fun withdraw_funds<CoinType>(
        ...
        assert!(exists<CoinStore<CoinType>>(@buggyswap), ERR_COINSTORE_NONEXISTENT);
        let coin = coin::withdraw<CoinType>(user, amount);
        let dest_coin_store = borrow_global_mut<CoinStore<CoinType>>(@buggyswap);
    ...
    fun deposit_funds<CoinType>(
        ...
        let coin_store = borrow_global_mut<CoinStore<CoinType>>(@buggyswap);
        ...
        coin::deposit(user_address, coin); // @audit VULN: does not check if user address is registered
    }
    ...
    fun execute_limit_order<QuoteCoinType>(
        ...
        //@audit Vuln: Unbounded Execution in drop_order: Can be called by cancel_order & execute_limit_order
    ...
    public entry fun admin_setup(admin: &signer) acquires OrderStore {
        ...
        admin_create_coinstore<BUG>(admin);
        admin_create_coinstore<USDC>(admin);
    }


    public entry fun admin_deposit_bug(admin: &signer, amount: u64) acquires OrderStore, CoinStore { 
        assert!(address_of(admin) == @buggyswap, ERR_PERMISSION_DENIED);
        assert!(coin::is_coin_initialized<BUG>(), ERR_COINSTORE_COIN_NOT_INITIALIZED);
        let order_store = borrow_global_mut<OrderStore>(@buggyswap);
        let coin = coin::withdraw<BUG>(admin, amount);
        let dest_coin_store = borrow_global_mut<CoinStore<BUG>>(@buggyswap);
        register_liquidity(order_store, amount, type_info::type_of<BUG>());
        coin::merge<BUG>(&mut dest_coin_store.coins, coin);
    }
    ...
    public entry fun admin_create_coinstore<CoinType>(
        ...
        //@audit Vuln: Improper admin verfication. Needs the following assert -- assert!(address_of(admin) == @buggyswap, ERR_PERMISSION_DENIED);
        assert!(coin::is_coin_initialized<CoinType>(), ERR_COINSTORE_COIN_NOT_INITIALIZED);
        assert!(!exists<CoinStore<CoinType>>(@buggyswap), ERR_COINSTORE_ALREADY_EXISTS);
    ...
    //@audit Vuln: Unauthorized Protocol Control. Can be paused by anyone.
    public entry fun pause_protocol(admin: &signer) {
        buggyswap::market::pause_protocol(admin);
    }
    //@audit Vuln: Unauthorized Protocol Control. Can be resumed by anyone.
    ...
    fun calculate_lp_coin_amount_internal(
        ...
        return size * get_usd_value_internal(order_store, type) //@audit Vuln: can be overflow
    }
    ...
    public fun calculate_lp_coin_amount(
        ...
        let order_store = borrow_global<OrderStore>(@buggyswap);
    ...
    public fun calculate_protocol_fees(
        ...
        return size * PROTOCOL_FEE_BPS / 10000 //@audit Vuln: Can be overflown 
    }

    //@audit Vuln: Unbounded Execution in get_order_by_id:--Functions cancel_order & fullfill_order calls this get_order_by_id
